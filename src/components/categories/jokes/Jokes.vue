  ...
  <div class="jokes-component">
    <transition name="button-dropdown-slide">
      <div v-if="showCategories" class="categoryChooser">
        <b-button
          v-on:click="showJoke(null)"
          v-bind:class="themeService.getActiveTheme().themeName"
        >{{$t('jokes.user.randomCategoryChosen')}}</b-button>
    ...
    <transition name="button-picker-slide" @after-enter="disabled=false">
      <div v-if="showNextSteps">
        <b-button
          :disabled="disabled"
          v-if="showRatingJokesBtn"
          v-on:click="disableButtons(showRatingComponent)"
          v-bind:class="themeService.getActiveTheme().themeName"
        >{{$t('jokes.user.rateJokeBtn')}}</b-button>
        <b-button
          :disabled="disabled"
          v-if="showCategoriesBtn"
          v-on:click="disableButtons(showAnotherJoke)"
          v-bind:class="themeService.getActiveTheme().themeName"
        >{{$t('jokes.user.giveNext')}}</b-button>
        <b-button
          :disabled="disabled"
          v-if="showNewJokeBtn"
          v-on:click="disableButtons(showNewJoke)"
          v-bind:class="themeService.getActiveTheme().themeName"
        >{{$t('jokes.user.tellJokeBtn')}}</b-button>
      </div>
    </transition>
    <RatingJoke
      v-bind:joke-id="shownJoke.id"
      v-if="showRatingJokesComponent"
      v-on:showRating="showUserRating($event)"
    ></RatingJoke>
  ...
  methods: {
    ...
    showJoke(category) {
      ...
      if (category == null) {
        this.sendMessageFromUser(
          this.$t("jokes.user.randomCategoryChosen")
        ).then(() => {
          jokesService.getRandomJoke().then(data => {
            if (data.errors) {
              this.$root.$emit(
                "showDanger",
                this.$t("jokes.errors.errorGetJoke") + data.errors[0]
              );
            } else {
              this.shownJoke = data;
              this.sendMessageFromBot(this.shownJoke.joke).then(() =>
                this.sendMessageFromBot(this.$t("jokes.bot.smile")).then(() =>
                  this.sendMessageFromBot(this.$t("jokes.bot.greatJoke")).then(
                    () =>
                      this.sendMessageFromBot(
                        this.$t("jokes.bot.howYouLikeIt")
                      ).then(() => {
        ...
        this.sendMessageFromUser(
          this.$t("jokes.user.categoryChosen") + category
        ).then(() => {
          jokesService.getJokeForCategory(category).then(data => {
            if (data.errors) {
              this.error =
                this.$t("adminJokes.errorGetJokeCategory") + data.errors[0];
    ...
    showRatingComponent() {
      ...
      this.sendMessageFromUser(this.$t("jokes.user.rateJoke")).then(() => {
        this.sendMessageFromBot(this.$t("jokes.bot.rateJokeResponse1")).then(
          () => {
            this.sendMessageFromBot(
              this.$t("jokes.bot.rateJokeResponse2")
            ).then(() => {
    ...
    showUserRating(rating) {
      ...
      this.sendMessageFromUser(
        rating + this.$t("jokes.user.outOf") + this.maxRate
      ).then(() => {
        if (rating <= 2.5)
          this.sendMessageFromBot(this.$t("jokes.bot.lowRate1")).then(() => {
            this.sendMessageFromBot(this.$t("jokes.bot.lowRate2")).then(() => {
