<template>
  <div>
    ...
    <b-form @submit="saveJoke" v-if="!showCategories">
      <b-form-input v-model="joke" type="text" required :placeholder="jokePlaceholder"></b-form-input>
    </b-form>
  </div>
</template>
...
export default {
  ...
  computed: {
    jokePlaceholder() {
      return this.$t("jokes.user.tellJokePlaceholder") + this.category + "...";
    }
  },
  methods: {
    ...
    saveJoke() {
      ...
      jokesService.createJoke(this.joke, this.category).then(data => {
        if (data.errors) {
          this.$root.$emit(
            "showDanger",
            this.$t("jokes.errors.errorCreatingJoke") + data.errors[0]
          );
        }
      });
